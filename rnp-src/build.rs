...
fn main() {
    ...
    // botan_src::build() configures and compiles Botan (as a static
    // library), returning (build_dir, include_dir). We then copy the
    // artifacts into our own prefix instead of relying on
    // `make install`, which is brittle on minimal images that lack
    // Sphinx/texi/etc. for the docs sub-target.
    ...
    std::fs::create_dir_all(botan_prefix.join("lib")).ok();
    std::fs::create_dir_all(botan_prefix.join("include")).ok();

    // Static library.
    let lib_src = PathBuf::from(&botan_build_dir).join("libbotan-3.a");
    if lib_src.exists() {
        std::fs::copy(&lib_src, botan_prefix.join("lib").join("libbotan-3.a"))
    ...
    // Install them at <prefix>/include/botan-3/ so cmake's find_package
    // finds them via BotanConfig.cmake (which Botan 3.6+ ships pointing
    // at include/botan-3).
    ...
    // Generate a minimal BotanConfig.cmake so find_package(Botan) succeeds
    // in librnp's cmake invocation. We skip `make install` because its
    // docs sub-target needs Sphinx, which may not be present.
    let cmake_dir = botan_prefix
        .join("lib")
        .join("cmake")
        .join(format!("Botan-{}", botan_src::BOTAN_VERSION));
    std::fs::create_dir_all(&cmake_dir).ok();
    let config = format!(
        "# Auto-generated by rnp-src\n\
         set(BOTAN_FOUND TRUE)\n\
         set(BOTAN_VERSION \"{ver}\")\n\
         set(BOTAN_LIBRARY \"{prefix}/lib/libbotan-3.a\")\n\
         set(BOTAN_INCLUDE_DIRS \"{prefix}/include/botan-3\")\n\
         add_library(Botan::Botan STATIC IMPORTED GLOBAL)\n\
         set_target_properties(Botan::Botan PROPERTIES\n\
             IMPORTED_LOCATION \"{prefix}/lib/libbotan-3.a\"\n\
             INTERFACE_INCLUDE_DIRECTORIES \"{prefix}/include/botan-3\")\n",
        ver = botan_src::BOTAN_VERSION,
        prefix = botan_prefix.display(),
    );
    std::fs::write(cmake_dir.join("BotanConfig.cmake"), config)
...
/// destination-already-exists, which is fine for our prefix-staging use.
fn copy_dir_recursive(src: &PathBuf, dst: &PathBuf) -> std::io::Result<()> {
    ...
    for entry in fs::read_dir(src)? {
        ...
        if file_type.is_dir() {
            ...
            // Preserve symlinks by copying their target — Botan headers are
            // regular files, so this branch rarely fires.
